...
ggplot(data = graph_set, mapping = aes(x = mdy(DATE_START), y = relative_ocd)) + geom_point() + geom_smooth() + ylim(50,250) + xlab("Date (2020)") + ylab("Search frequency for 'ocd', 100 = 2019 mean") + labs(title="OCD Searches Spiked In First COVID Wave")
...
--may need to redo some models?
Anxiety in USA, OCD in Brazil, depression in Mexico are not causes for concern

Therapy in Brazil (skewed right) and insomnia in Mexico (okay, not great) and insomnia in India (skewed left) and therapy in USA (okay, not great) may indicate need for another model.
